...
async function main() {
  ...
  await run('img -> inline svg, data-hmk-from/to/puml copied to the root', async () => {
    ...
    assert.ok(/<svg\b[^>]*data-hmk-puml/.test(out), 'puml marker not stamped on svg root: ' + out);
  ...
  await run('works without data-hmk-from/to (no data-line on the fence)', async () => {
    ...
    const out = await inlinePlantumlSvgs(html, stubFetcher({ 'http://localhost:9274/svg/abc': SVG }));
    assert.ok(/<svg\b[^>]*xmlns="http:\/\/www\.w3\.org\/2000\/svg"/.test(out), 'no svg: ' + out);
  ...
  await run('svg without xml declaration', async () => {
    ...
    assert.ok(/<svg\b[^>]*xmlns="http:\/\/www\.w3\.org\/2000\/svg"[^>]*width="100"[^>]*height="50"[^>]*><\/svg>/.test(out), 'bad svg: ' + out);
  });
  ...
  await run('all inlined, order preserved, non-puml between kept', async () => {
    ...
    const svgHas = (id, from, to) =>
      new RegExp(`<svg\\b(?=[^>]*id="${id}")(?=[^>]*data-hmk-from="${from}")(?=[^>]*data-hmk-to="${to}")(?=[^>]*data-hmk-puml)[^>]*>`).test(out);
  ...
  await run('one failing fetch, one success', async () => {
    ...
    const out = await inlinePlantumlSvgs(html, fetcher);
    assert.ok(/<svg\b[^>]*id="OK"/.test(out), 'ok diagram not inlined: ' + out);

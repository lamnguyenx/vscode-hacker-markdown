...
- **PlantUML without the plantuml extension** — `puml`/`plantuml`/`uml` fences render as PlantUML-server SVGs (needs `hackerMarkdown.plantuml.server`); `!include` resolves relative to the Markdown file. Scoped to this preview only — the stock preview is untouched.
